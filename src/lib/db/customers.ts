...
export async function searchCustomers(
  ...
  const { data, error } = await supabase
    .from("customers")
    .select(
      "id, customer_code, username, house_id, full_name, area_id, phone, cnic, address_value, status, area:areas(id, name), package:packages(name)",
    )
    .or(
      `full_name.ilike.%${safeQuery}%,customer_code.ilike.%${safeQuery}%,username.ilike.%${safeQuery}%,phone.ilike.%${safeQuery}%,house_id.ilike.%${safeQuery}%`,
    )
    .order("customer_code")
    .limit(limit);
  if (error) throw error;
