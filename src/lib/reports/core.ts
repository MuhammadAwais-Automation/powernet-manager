...
export function getAreaScopeLabel(areaId?: string, areaName?: string): string {
  return areaId ? areaName?.trim() || "Selected Area" : "All Areas";
}
...
export function getReportChart(
  ...
  const dataByReport: Record<ReportType, ReportChartPoint[]> = {
    ...
    Customers: summary.customerGrowthMonths?.length
      ? summary.customerGrowthMonths
      : summary.customersMonths,
  };
